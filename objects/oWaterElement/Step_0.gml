...
if(state == enemy_state2.idle){
	...
	if(collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy_state2.alert; counter = 0;
	}
}

else if(state == enemy_state2.wander){
	image_blend = c_green
	x += move_speed;
	counter += 1;
...
else if(state == enemy_state2.alert){
	image_blend = c_red
	counter +=1
	...
	if(!collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy_state2.idle; counter = 0;
	}
}
