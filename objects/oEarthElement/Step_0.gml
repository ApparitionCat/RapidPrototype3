...
if(state == enemy1_state.idle){
	...
	if(counter >= room_speed * 2){
		...
		switch(change){
			case 0: state = enemy1_state.wander;
			case 1: counter = 0; break;
		}
	}
	
	if(collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy1_state.alert; counter = 0;
	}
}

else if(state == enemy1_state.wander){
	image_blend = c_green
	x += move_speed;
	counter += 1;
...
else if(state == enemy1_state.alert){
	image_blend = c_red
	counter +=1
	if(counter >= room_speed*1){
		var proj = instance_create_layer(x,y,"Instances", pBoulder);
		proj.direction = point_direction(x,y,oPlayer.x,oPlayer.y)
		counter = 0
	}
	
	if(!collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy1_state.idle; counter = 0;
	}
}