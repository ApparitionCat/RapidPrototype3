...
if(state == enemy_state3.idle){
	...
	if(counter >= room_speed * 2){
		...
		switch(change){
			case 0: state = enemy_state2.wander;
			case 1: counter = 0; break;
		}
	}
	
	if(collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy_state3.alert; counter = 0;
	}
}

else if(state == enemy_state3.wander){
	image_blend = c_green
	x += move_speed;
	counter += 1;
	if(counter >= room_speed * 4){
		...
		var change = choose(0,1);
		switch(change){
			case 0: state = enemy_state2.idle; counter = 0; break;
			case 1:
			   spd = choose(-1.7,1.7);
			   move_speed = spd;
			   counter = 0; break;
...
else if(state == enemy_state3.alert){
	image_blend = c_red
	counter +=1
	...
	if(!collision_circle(x,y,200,oPlayer,false,false)){
		state = enemy_state3.idle; counter = 0;
	}
}
